...
import ViewCrewDetailsModal from '../../CrewingTest/components/ViewCrewDetailsModal'
...
const CrewDetails: FC = () => {
    ...
    /** ---- Vessel helpers ---- */
const getCrewVesselId = (c: Crew) => c?.vessel?.id ?? c?.vesselId ?? null;
/** On this page we show same-vessel only */
const belongsToUserVessel = (c: Crew) => {
  if (!vesselId) return false;           // enforce same-vessel only when user has a vessel
  return getCrewVesselId(c) === vesselId;
};
    ...
    useEffect(() => {
        ...
        try {
            ...
            const sameVessel = crewList.filter(belongsToUserVessel);
...
const compareRank = (a?: string, b?: string, dir: 'asc'|'desc' = 'asc'): number => {
  ...
  .filter((c) => {
    // must belong to the current user's vessel
    if (!belongsToUserVessel(c)) return false;

    const matchesActiveStatus =
      selectedActiveStatus === 'all' ||
      (selectedActiveStatus === 'active' && c.active) ||
      (selectedActiveStatus === 'inactive' && !c.active);

    return c.name?.toLowerCase().includes(searchTerm.toLowerCase()) && matchesActiveStatus;
  })
  .sort((a, b) => {
    // newest first by id if no explicit sort
...
const clearFilters = () => {
  ...
  const srNo = indexOfFirstCrew + idx + 1;
  return (
    <tr key={crew.id}>
        ...
        `}
                                                ...
                                                <table className="imo-table">
                                                    <thead>
                                                            ...
                                                            <td colSpan={6} className="top-header-cell">
                                                                <div style={{ display: 'flex', justifyContent: 'flex-start', gap: '15px', marginBottom: '5px' }}>
                                                                    <label className="checkbox-label">
                                                                        <input type="checkbox" className="form-checkbox" style={{ verticalAlign: 'middle' }} />
                                                                        <span>Arrival</span>
                                                                    </label>
                                                                    <label className="checkbox-label">
                                                                        <input type="checkbox" className="form-checkbox" style={{ verticalAlign: 'middle' }} />
                                                        ...
                                                        {/* Section 2 - 5 Data */}
                                                        <tr>
                                                            <td colSpan={4}>{headerData.portOfArrival || ''}</td>
                                                            <td colSpan={3}>
