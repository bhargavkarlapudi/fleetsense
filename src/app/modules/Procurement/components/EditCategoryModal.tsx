...
const EditCategoryModal: FC<EditCategoryModalProps> = ({ visible, onClose, onSubmit, category }) => {
  ...
  useEffect(() => {
    if (visible && category) {
      setFormData({
        name: category.name || '',
        company: category.cgaid?.id?.toString() || '',
        vesselType: typeof category.vesselType === 'string' ? category.vesselType : category.vesselType?.fleet_name || ''
      })
    }
  }, [visible, category])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateInventoryCategory(category.id, {
        name: formData.name,
        cgaid: { id: cgaIdToUse },
        caid: { id: null },
              ...
              {actsAsCga && effectiveCgaId && (
                <div className='col-12'>
                  <div className='alert alert-info'>
                    ...
                    Category will be updated under your Company Group Admin (ID: {effectiveCgaId})
                  </div>
                </div>
              )}
            </div>
