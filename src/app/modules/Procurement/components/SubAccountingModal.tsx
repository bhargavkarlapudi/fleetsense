...
interface SubAccountingModalProps {
  visible: boolean
  onClose: () => void
  onSubmit: () => void
}

const SubAccountingModal: FC<SubAccountingModalProps> = ({ visible, onClose, onSubmit }) => {
  ...
  // New state for company and sub company dropdowns
  ...
  // Load accounting codes when modal opens or when company/vessel type changes
  useEffect(() => {
    if (visible) {
      loadAccountingCodes()
    }
  ...
  const loadAccountingCodes = async () => {
    ...
    try {
      ...
      // Use selected vessel type
      const vesselTypeToUse = formData.vesselType

      const data = await getAccountingAccountsWithParams(cgaIdToUse, vesselTypeToUse)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('Sub Accounting added successfully!')
      onSubmit()
      handleClose()
    } catch (error: any) {
      console.error('Error adding sub accounting:', error)
      toast.error(error.message || 'Failed to add sub accounting')
    } finally {
      ...
      <div className='modal-dialog modal-lg modal-dialog-centered' role='document'>
        <div className='modal-content bg-white' style={{ color: '#181C32' }}>
          <div className='modal-header'>
            <h5 className='modal-title'>Add Sub Accounting</h5>
            ...
            <div className='row g-3'>
              ...
              <div className='col-md-6'>
                ...
                <select
                  ...
                  onChange={handleInputChange}
                  disabled={isLoadingVessels || (currentUser?.role?.id === 1 && !formData.company)}
                >
                  <option value=''>
              ...
              <div className='col-md-6'>
                ...
                <select
                  ...
                  onChange={handleInputChange}
                  disabled={isLoadingAccounts || (currentUser?.role?.id === 1 && (!formData.company || !formData.vesselType)) || (currentUser?.role?.id !== 1 && !formData.vesselType)}
                >
                  <option value=''>
                    {isLoadingAccounts ? 'Loading account codes...' :
                      (currentUser?.role?.id === 1 && !formData.company) ? 'Please select a company and vessel type first' :
                        (currentUser?.role?.id === 1 && !formData.vesselType) ? 'Please select a vessel type first' :
          ...
          <div className='modal-footer'>
            ...
            <button type='button' className='btn btn_primary' onClick={handleSubmit} disabled={isSubmitting}>
              {isSubmitting ? 'Adding...' : 'Add Sub Accounting'}
            </button>
          </div>
        </div>
      </div>
...
export default SubAccountingModal;