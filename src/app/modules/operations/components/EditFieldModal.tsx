...
import { updateField } from '../core/_requests';
...
const EditFieldModal: React.FC<Props> = ({ onClose, isOpen, fieldData, onFieldEdited }) => {
    ...
    useEffect(() => {
        if (fieldData) {
            setLabelName(fieldData.label);
            setInputType(fieldData.fieldType);
            setMinValue(fieldData.minValue);
            setMaxValue(fieldData.maxValue);
            setUnit(fieldData.unit || "");
            setOptions(fieldData?.optionsJson ?? []);
            setWarningOnly(fieldData.warningOnly ?? null);
            setActive(fieldData.isActive);
            setIsRequired(fieldData.required);
            setIsReadOnly(fieldData.readOnly);
        }
    }, [fieldData]);

    const handleEditFieldItem = async () => {
        try {
          

            console.log(fieldData);
            if (!fieldData || !fieldData.id || !fieldData.submenuId) {
                console.error('Missing required data to update field');
                return;
            }

            await updateField(
                fieldData.submenuId, // submenu ID
        ...
        <div className="modal-overlay">
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="custom-modal-body">
                    <div style={{ paddingInline: '2.5rem', marginBottom: '1rem' }}>
                        ...
                        {inputType === "number" || inputType === "text" && (
                            <div className="mt-4">
                                <label className="modal_label">Validation Range</label>
                        ...
                        {!isReadOnly && <div className='d-flex align-items-center mt-5'>
                            ...
                            />
                            <label className="modal_label ps-3 mb-0">
                                Required
                            </label>
                        </div>}
                    </div>
                ...
                <div className="d-flex justify-content-end gap-2">
                    ...
                    <button className="btn btn_primary" onClick={handleEditFieldItem}>Update</button>
                </div>
            </div>
        </div>
    );
