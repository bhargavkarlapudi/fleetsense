...
import InspectionFindingAttachmentsModal from './InspectionFindingAttachmentsModal'
...
import {
  searchInspectionFindings,
  createInspectionFinding,
  getInspectionFinding,
  updateInspectionFinding,
  uploadInspectionFindingAttachments,
  listInspectionFindingAttachments,
  inspectionFindingAttachmentViewUrl,
  inspectionFindingAttachmentDownloadUrl,
  inspectionFindingAttachmentFileViewUrl,
  inspectionFindingAttachmentFileDownloadUrl,
  searchInspections,
  deleteInspectionFindingAttachment,
} from '../core/_requests'

import AddInspectionFindingModal from './AddInspectionFindingModal'
import {ViewInspectionFindingModal} from './ViewInspectionFindingModal'
import EditInspectionFindingModal from './EditInspectionFindingModal'
import type {FindingSubmitShape} from '../core/_models'
...
const InspectionFindingIndex: FC = () => {
...
const [filters, setFilters] = useState({
  ...
  // inspection dropdown for Add/Edit
  const [inspectionOptions, setInspectionOptions] = useState<{id: number; label: string}[]>([])

  // data
  const [records, setRecords] = useState<InspectionFindingRecord[]>([])
  ...
  const [sortColumn, setSortColumn] = useState<keyof InspectionFindingRecord | ''>('findingDate')
  ...
  const [viewing, setViewing] = useState<QHSEInspectionFindingDto | null>(null)
  const [editing, setEditing] = useState<QHSEInspectionFindingDto | null>(null)
  ...
  // -----------------------
  // Load inspection options for dropdown (finalized inspections)
  // -----------------------
  const loadInspectionOptions = async () => {
    try {
      ...
      const list: InspectionDto[] = Array.isArray(res?.content)
        ? res.content
        : (Array.isArray(res) ? res : [])

      const options = list.map((d: InspectionDto) => {
        const vesselMeta = d.vesselId ? vesselMetaMap[d.vesselId] : undefined
        const vName = d.vesselName || vesselMeta?.name || '-'
        const labelParts = [
          vName,
          d.inspectionKindName || d.inspectionType || '',
          d.inspectionFromDate || d.inspectionToDate || '',
        ].filter(Boolean)
      ...
      console.error('Failed to load inspection options', e)
...
/** we treat "finding date" in UI as the inspection date now */
const deriveInspectionDate = (d: QHSEInspectionFindingDto): string | null => {
  // Prefer inspectionFromDate, fall back to inspectionToDate
  const raw =
    (d as any).inspectionFromDate ||
    (d as any).inspectionToDate ||
    null
...
const mapDtoToRecord = (d: QHSEInspectionFindingDto): InspectionFindingRecord => {
  ...
  return {
    ...
    inspectionName: d.inspectionKindName || d.inspectionType || '-',
    findingLabel: label,
    findingName: textDetails || label || '',
    critical: (d as any).critical ?? (d as any).isPositive ?? null,
    status: d.status,
    findingType: d.findingType ?? null,
    // 🔁 this is now INSPECTION DATE
    findingDate: deriveInspectionDate(d),
  ...
  activeOnly: undefined,
      ...
      const list = await searchInspectionFindings(params)
      setRecords(list.map(mapDtoToRecord))
    } catch (e) {
      console.error('Failed to load inspection findings', e)
      setRecords([])
      toast.error('Failed to load inspection findings', {position: 'top-center'})
    } finally {
      setLoading(false)
    }
  }
  ...
  const filteredData = useMemo(() => {
    return records.filter(r => {
      const matchesSearch =
        ...
        (r.inspectionName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
...
const columnCount = showVesselColumn ? 11 : 10   
  ...
  const handleSort = (col: keyof InspectionFindingRecord) => {
    setSortColumn(col)
    setSortOrder(prev =>
  ...
  const openView = async (id: number) => {
    try {
      const dto = await getInspectionFinding(id)
      setViewing(dto)
      setViewOpen(true)
    } catch (e) {
  ...
  const openAttachmentsModal = async (rec: InspectionFindingRecord | QHSEInspectionFindingDto) => {
  ...
  try {
    const list: InspectionFindingAttachmentInfo[] = await listInspectionFindingAttachments(id)
    const items = list.map(it => ({
      fileName: it.fileName,
      viewUrl: inspectionFindingAttachmentFileViewUrl(id, it.fileName),
      downloadUrl: inspectionFindingAttachmentFileDownloadUrl(id, it.fileName),
    }))
  ...
  const handleAddFinding = async (form: FindingSubmitShape) => {
  const payload: Partial<QHSEInspectionFindingDto> & {
  inspectionId: number
  findingName: string
  status: FindingStatus
} = {
  inspectionId: Number(form.inspectionId),
  findingName: form.findingName,
  description: form.description,
  // 🔁 no findingDate – date comes from inspection itself
  isPositive:
  ...
  try {
    const created = await createInspectionFinding(payload)
    const id = Number(created.id)
    if (id && form.attachmentFiles && form.attachmentFiles.length > 0) {
      await uploadInspectionFindingAttachments(id, form.attachmentFiles)
    }
    toast.success('Inspection finding created successfully', {position: 'top-center'})
    setAddOpen(false)
    await loadFindings()
  } catch (e: any) {
    console.error('Create inspection finding failed', e)
    toast.error(
  ...
  const handleUpdateFinding = async (form: FindingSubmitShape) => {
    if (!editing?.id) return

    const partial: Partial<QHSEInspectionFindingDto> = {
  inspectionId: Number(form.inspectionId),
  findingName: form.findingName,
  description: form.description,
  // 🔁 no findingDate – inspection date is already tied to the inspection
  isPositive:
    form.critical === 'YES' ? true :
    form.critical === 'NO' ? false : undefined,
  remarks: form.remarks || undefined,
  ...
  findingType: form.findingType as any,
    ...
    try {
      const updated = await updateInspectionFinding(editing.id, partial)
      const id = Number(updated.id)
      if (id && form.attachmentFiles && form.attachmentFiles.length > 0) {
        await uploadInspectionFindingAttachments(id, form.attachmentFiles)
      }

      toast.success('Inspection finding updated successfully', {position: 'top-center'})
      setEditOpen(false)
      setEditing(null)
      await loadFindings()
    } catch (e: any) {
      console.error('Update inspection finding failed', e)
      toast.error(
        ...
        <div id='kt_app_content' className='app-content flex-column-fluid bg-white'>
          <div className='card'>
            ...
            <div className='card-header border-0 pt-6 d-flex justify-content-between bg-white'>
              <div>
                <h3 className='card-label text-dark fw-bold'>Inspection Findings</h3>
              </div>
    ...
    <option value='DEVIATION_NOTE'>Deviation note</option>
                ...
                <div className='col-md-2'>
                  ...
                  <input
                    ...
                    placeholder='Vessel / inspection / finding'
                  />
                </div>
                    ...
                    <thead className='table-header text-start'>
                      <tr>
                        ...
                        <th style={{minWidth: '200px'}}>
                          <div className='d-flex align-items-center text-nowrap'>
                            <span className='me-1'>Inspection</span>
                            <button
                              type='button'
                              className='btn btn-link p-0 m-0 pb-1'
                              onClick={() => handleSort('inspectionName')}
                              disabled={records.length === 0}
                            >
                              <KTSVG
                                path={`/media/map/sort-col-${
                                  sortColumn === 'inspectionName'
                                    ? sortOrder === 'asc'
                                      ? 'up-black'
                                      : 'down-black'
                                    : 'grey'
...
<th style={{minWidth: '140px'}}>
  <div className='d-flex align-items-center text-nowrap'>
    <span className='me-1'>Inspection Date</span>
                            <button
                              type='button'
                              className='btn btn-link p-0 m-0 pb-1'
                      ...
                      {(bootLoading || loading) ? (
                        ...
                        <tr>
                          <td colSpan={columnCount} className='text-center py-5 text-muted'>
                            No inspection findings found for the selected criteria.
                          </td>
                        </tr>
                      ) : (
...
<td className='text-nowrap'>
  {r.findingType ? niceEnum(r.findingType) : '-'}
      ...
      />
            ...
            <InspectionFindingAttachmentsModal
...
export default InspectionFindingIndex
