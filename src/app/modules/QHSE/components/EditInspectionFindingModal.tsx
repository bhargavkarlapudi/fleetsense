...
interface EditInspectionFindingModalProps {
  visible: boolean
  onClose: () => void
  record: QHSEInspectionFindingDto | null
  inspections: InspectionOption[]
  onSubmit: (data: FindingSubmitShape) => void | Promise<void>
}
...
const EditInspectionFindingModal: FC<EditInspectionFindingModalProps> = ({
  visible,
  onClose,
  record,
  inspections,
  ...
  const [form, setForm] = useState<FindingSubmitShape>({
    inspectionId: '',
    findingName: '',
    description: '',
    critical: '',
  ...
  useEffect(() => {
    if (visible && record) {
      ...
      setForm({
        inspectionId: record.inspectionId ? String(record.inspectionId) : '',
        findingName: record.findingName || '',
        description: record.description || '',
        // 🔁 no separate finding date – it is derived from inspection
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const list = await listInspectionFindingAttachments(findingId)
        const mapped: ExistingAttachmentLocal[] = list.map((it: any) => ({
          fileName: it.fileName,
          viewUrl: inspectionFindingAttachmentFileViewUrl(findingId, it.fileName),
          downloadUrl: inspectionFindingAttachmentFileDownloadUrl(findingId, it.fileName),
        }))
  ...
  const handleDeleteExistingFile = async (fileName: string) => {
    ...
    try {
      await deleteInspectionFindingAttachment(Number(record.id), fileName)
  ...
  const validate = () => {
    ...
    if (!form.inspectionId.trim()) er.inspectionId = 'Inspection is required'
    ...
    <div style={WRAP_STYLE} tabIndex={-1}>
      ...
      >
        ...
        <div className='d-flex align-items-center justify-content-between border-bottom px-4 py-3 flex-shrink-0'>
          <h5 className='modal-title text-dark m-6'>Edit Inspection Finding</h5>
          ...
          <div className='row g-4'>
            ...
            <div className='col-md-6'>
              <label className={LABEL}>
                Inspection <span className='text-danger ms-1'>*</span>
              </label>
              <select
                name='inspectionId'
                className={`form-select ${errors.inspectionId ? 'is-invalid' : ''}`}
              ...
              {errors.inspectionId && (
                <div className='invalid-feedback'>{errors.inspectionId}</div>
              )}
            </div>
...
export default EditInspectionFindingModal
