...
import {
  listManualPlanRevisions,
  manualPlanRevisionDownloadUrl,
  manualPlanRevisionViewUrl,
} from '../core/_requests'
...
export const ManualPlanRevisionsModal: FC<{
  manualId: number
  name: string
  visible: boolean
  onClose: () => void
}> = ({ manualId, name, visible, onClose }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const data = await listManualPlanRevisions(manualId)
  ...
  const handleDownload = async (rev: RevRow) => {
    try {
        const res = await fetch(manualPlanRevisionDownloadUrl(rev.id), {
  ...
  <table className='table table-bordered align-middle m-0'>
    <thead className='table-header text-start'>
      <tr>
        ...
        <th className='text-nowrap' style={{ minWidth: '100px' }}>REVISION NO</th>
        {/* <th className='text-nowrap' style={{ minWidth: '160px' }}>UPLOADED BY</th> */}
        <th className='text-nowrap' style={{ minWidth: '180px' }}>UPLOADED DATE</th>
        {/* <th className='text-nowrap' style={{ minWidth: '220px' }}>FILE NAME</th> */}
        {/* <th className='text-nowrap' style={{ minWidth: '120px' }}>FILE SIZE</th> */}
        {/* <th style={{ minWidth: '240px' }}>REMARKS</th> */}
      ...
      {currentRows.length === 0 ? (
        ...
        currentRows.map((r, idx) => (
          <tr key={r.id}>
            ...
            <td className='text-dark fs-6 text-nowrap'>
              {formatDate(r.uploadedDate)}
            </td>

            {/* <td className='text-dark fs-6 text-truncate' title={r.fileName || ''}>
              {r.fileName || '-'}
            </td> */}
            ...
            <td className='text-gray-600 fs-7'>
              <div title={r.remarks || ''} className='text-truncate' style={{maxWidth: 360}}>
                {r.remarks || '-'}
              </div>
            </td> */}
            ...
            <td className='text-center'>
              <div className='d-flex align-items-center justify-content-center'>
                <button
                  ...
                  title='View'
                  onClick={() => setViewingRev({...r, fileName: r.fileName || `${name}`})}
                >
...
{/* ====== END: Page-like pagination ====== */}
          ...
          {/* Footer */}
          {/* <div className="border-top bg-white" style={{ padding: '10px 16px', flexShrink: 0 }}>
      ...
      {viewingRev && (
        <ManualPlanViewer
          ...
          viewUrlOverride={manualPlanRevisionViewUrl(viewingRev.id)}
          downloadUrlOverride={manualPlanRevisionDownloadUrl(viewingRev.id)}
