...
import type {FindingSubmitShape, FindingStatus} from '../core/_models'
...
interface AddInspectionFindingModalProps {
  visible: boolean
  onClose: () => void
...
const AddInspectionFindingModal: FC<AddInspectionFindingModalProps> = ({
  visible,
  onClose,
  onSubmit,
  inspections,
  ...
  // auto-select when only one inspection
  useEffect(() => {
    if (!form.inspectionId && inspections.length === 1) {
      setForm(prev => ({...prev, inspectionId: String(inspections[0].id)}))
    }
  ...
  useEffect(() => {
    if (visible) {
      setForm({
        inspectionId: inspections.length === 1 ? String(inspections[0].id) : '',
        findingName: '',
        description: '',
        critical: '',
        status: '' as '' | FindingStatus,
    ...
    <div style={WRAP_STYLE} tabIndex={-1}>
      ...
      >
        ...
        <div className='d-flex align-items-center justify-content-between border-bottom px-4 py-3 flex-shrink-0'>
          <h5 className='modal-title text-dark m-6'>Add Inspection Finding</h5>
...
export default AddInspectionFindingModal
