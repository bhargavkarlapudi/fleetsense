...
import {
  listAuditKinds,
  listAuditPlans,
  bulkCreateAuditPlans,      // ✅ use audit-specific bulk create
  getAudit,
  getAuditPlan,
  listPorts,
  listCrewsLite,
...
import AddAuditPlanModal from './AddAuditPlanModal'
import { AuditPlanActionsModal } from './AuditPlanActionsModal'
...
type NewPlanData = {
  auditName: string        // kindId as string
...
const getPlanStatus = (plan: AuditPlan): 'overdue' | 'planned_done' | 'next_30_days' | 'upcoming' => {
...
// --- MAIN AUDIT PLANNING COMPONENT ---
const AuditPlanning: FC = () => {
  ...
  // crews for internal-Auditor fallback (same shape as Index)
...
const [selectedPlan, setSelectedPlan] = useState<AuditPlanDto | null>(null)
  ...
  // to open AddAudit from a plan, prefilled:
 ...
 const toViewPlan = useMemo(() => {
  return (p: AuditPlanDto): AuditPlan => ({
    id: p.id,
    vesselName:
      ...
      `-`,
    auditName:
      kindMap[p.auditKindId] ||
      (p.auditKindName && p.auditKindName.trim()) ||
      `-`,
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // kinds list -> map
        const kindsList: QhseAuditKind[] = Array.isArray(k) ? k : []
        ...
        // ⬇️ Crews list for internal Auditor fallback (same helper used in Index)
        try {
          ...
          console.warn('Failed to load crews for internal auditor fallback', e)
          setCrews([])
        }
  ...
  useEffect(() => {
    (async () => {
      const [k] = await Promise.all([listAuditKinds()]);
  ...
  const loadPlans = async () => {
    try {
      const list = await listAuditPlans({ year: filters.year, companyGroupId: effectiveCompanyId })
...
const openViewAuditFromPlan = async (auditId: number) => {
  try {
    const dto = await getAudit(auditId)
    ...
    console.error('Failed to load audit for viewing', e)
  }
}
  ...
  const handleAddPlan = async (data: NewPlanData, _saveAndAddNew: boolean) => {
    ...
    await bulkCreateAuditPlans({
      year: data.year,
      auditKindId: Number(data.auditName),
  ...
  const handleBulkSubmit = async (data: { year:number; auditKindId:number; vesselIds:number[]; months:number[] }, saveAndAddNew:boolean) => {
    await bulkCreateAuditPlans({
      year: data.year,
      auditKindId: data.auditKindId,
        ...
        <div id='kt_app_content' className='app-content flex-column-fluid bg-white'>
          <div className='card'>
            ...
            <div className='card-header border-0 pt-6 d-flex justify-content-between bg-white'>
              <div>
                <h3 className='card-label text-dark fw-bold'>Audit Planning</h3>
              </div>
                              ...
                              {(plansByVesselAndMonth[vessel]?.[month] ?? []).map(plan => {
                                ...
                                return (
                                  <button
                                    ...
                                    title="Plan actions"
                                  >
                                    <span>{plan.auditName}</span>
                                    {dto?.auditId ? <span className='badge bg-success'>Linked</span> : <span className='badge bg-secondary'>Unlinked</span>}
                                  </button>
                                );
                              })}
                            </td>
...
/>
  ...
  onViewAudit={(auditId) => {
    ...
    openViewAuditFromPlan(Number(auditId))
  }}
  onCreateAndLink={(prefill) => {
    setPlanActionsOpen(false)
    setAddFromPlan(prefill)       // this alone shows AddAuditModal (visible={Boolean(addFromPlan)})
  }}
/>
...
{addFromPlan && (
  ...
  auditKindId: Number(data.audit),
  auditType: (data.auditType as any) || 'DETAILS_TYPE',
  auditFromDate: data.auditDate,
  auditToDate: data.auditDate,
  internalAuditorName: data.internalAuditorName || undefined,
  ...
  remarks: data.auditRemarks || undefined,
} as any

      try {
        const created = await createAudit(payload)
        if (!created?.id) throw new Error('Create audit failed')

        await linkAuditPlanToAudit(Number(addFromPlan.planId), Number(created.id))

        await loadPlans()
      } catch (err) {
        console.error('Create+Link audit failed', err)
      } finally {
        setAddFromPlan(null)
      }
    }}
    ...
    prefill={{
      ...
      auditKindId: addFromPlan.auditKindId,
