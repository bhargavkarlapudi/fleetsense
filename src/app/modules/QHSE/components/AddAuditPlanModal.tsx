...
import type {QhseAuditKind, VesselLite} from '../core/_models'
...
interface AddAuditPlanModalProps {
  visible: boolean
  onClose: () => void
...
const AddAuditPlanModal: FC<AddAuditPlanModalProps> = ({
  visible,
  onClose,
  onSubmit,
  kinds,
  ...
  const vesselOptions: Option[] = vessels.map(v => ({
    value: String(v.id),
    label: v.name,
  }))
  const monthOptions: Option[] = (ALL_MONTHS as readonly string[]).map(m => ({
    value: m,
    label: m,
  }))
  ...
  const validate = () => {
    ...
    if (!auditKind) er.auditKind = 'Audit is required'
  ...
  const submit = (saveAndAddNew: boolean) => {
    if (!validate()) return
    const months = selectedMonthLabels
      .map(m => ALL_MONTHS.indexOf(m as any) + 1)
      .filter(n => n > 0)

    onSubmit(
      {
        year,
        auditKindId: Number(auditKind),
      ...
      <div className='modal-dialog modal-lg modal-dialog-centered'>
        <div className='modal-content bg-white text-dark'>
          <div className='modal-header'>
            <h5 className='modal-title text-dark'>Add Audit Plan</h5>
          ...
          <div className='modal-body'>
            <div className='row g-5'>
              ...
              <div className='col-12'>
                ...
                {errors.auditKind && (
                  <div className='invalid-feedback'>{errors.auditKind}</div>
                )}
              </div>

              <div className='col-md-6'>
                <label className={LABEL_CLS}>
                  Vessels <span className='text-danger'>*</span>
                </label>
                <MultiSelect
                  label='Vessels'
...
export default AddAuditPlanModal
