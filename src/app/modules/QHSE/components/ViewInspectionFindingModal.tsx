...
import type {QHSEInspectionFindingDto} from '../core/_models'

interface ViewInspectionFindingModalProps {
  visible: boolean
  onClose: () => void
  record: QHSEInspectionFindingDto | null
  onOpenAttachments?: (rec: QHSEInspectionFindingDto) => void
}
...
const ViewInspectionFindingModal: FC<ViewInspectionFindingModalProps> = ({
  visible,
  onClose,
  record,
  onOpenAttachments,
  ...
  // 🔁 treat "finding date" as inspection date now (from / to)
  const inspectionDateLabel = (() => {
    const from = (record as any).inspectionFromDate as string | undefined
    const to = (record as any).inspectionToDate as string | undefined
  ...
  const linkedInspectionText = (() => {
    const vessel = record.vesselName || '-'
    const kindOrType =
      record.inspectionKindName ||
      (record.inspectionType ? niceEnum(String(record.inspectionType)) : '-')

    return `${vessel} — ${kindOrType} — ${inspectionDateLabel}`
  })()
    ...
    <div style={WRAP_STYLE} tabIndex={-1}>
      ...
      >
        ...
        <div className='d-flex align-items-center justify-content-between border-bottom px-4 py-3 flex-shrink-0'>
          <h5 className='modal-title text-dark m-6'>Inspection Finding Details</h5>
        ...
        <div className='flex-grow-1 overflow-auto px-4 py-3 m-6'>
          ...
          <div className='row g-3'>
            ...
            <div className='col-md-3'>
              <div className='text-muted fw-semibold'>Inspection Date</div>
              <div>{inspectionDateLabel}</div>
            </div>
            ...
            <div className='col-md-6'>
              <div className='text-muted fw-semibold'>Internal Inspector</div>
              <div>{record.internalInspectorName || '-'}</div>
            </div>

            <div className='col-md-6'>
              <div className='text-muted fw-semibold'>External Inspector</div>
              <div>{record.externalInspectorName || '-'}</div>
            </div>
...
export {ViewInspectionFindingModal}
