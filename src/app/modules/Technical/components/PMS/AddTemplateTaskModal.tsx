...
import { createPmsTemplateTask } from '../../core/pms/_requests'
...
export const AddTemplateTaskModal: FC<Props> = ({
  visible,
  templateId,
  templateName,
  onClose,
  ...
  const [formData, setFormData] = useState<Partial<PmsTemplateTaskDto>>({
    templateId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.referenceId) newErrors.referenceId = 'Reference is required'
    ...
    try {
      ...
      await createPmsTemplateTask(templateId, payload)
      toast.success('Template task created successfully', { position: 'top-center' })
      onSuccess()
    } catch (error: any) {
      console.error('Error creating template task:', error)
      toast.error(error.message || 'Failed to create template task', { position: 'top-center' })
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div style={WRAP_STYLE} tabIndex={-1} onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      >
        <div className='d-flex align-items-center justify-content-between border-bottom px-4 py-3 flex-shrink-0'>
          <h5 className='modal-title text-dark m-0'>Add Task to Template: {templateName}</h5>
        ...
        <form onSubmit={handleSubmit}>
          <div className='flex-grow-1 overflow-auto px-4 py-3'>
            <div className='row g-3'>
              ...
              <div className='col-md-6'>
                <label className={LABEL}>Reference <span className='text-danger'>*</span></label>
