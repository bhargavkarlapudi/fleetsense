...
import {
  ...
  getNextOfKin, createNextOfKin, updateNextOfKin,
  ...
  updateSeaService,
  deleteSeaService ,
  getAdditionalDetails, updateAdditionalDetails, createAdditionalDetails,
} from '../core/_requests'
import {
  Rank,
  CompanyAdmin,
...
const DRAFT_KEY = 'addCrewWizardDraft_v1'
...
interface Props {
  isOpen: boolean
  onClose: () => void
  onCrewAdded: (payload: { password: string; link: string; created: Crew }) => void
}
...
// clears the top-of-form error banner
const clearTopError = React.useCallback(() => {
  setError('');
}, []);
...
const [references, setReferences] = useState<Reference[]>([{ srNo: 1, companyName: '', pic: '', designation: '', phoneNo: '' }]);
...
const [existingAdditionalDetails, setExistingAdditionalDetails] = useState<AdditionalDetailsResponse | null>(null);
...
const isSuperadminOperator = isOperator && (cgaId == null)      // acts like Superadmin
const isCompanyOperator   = isOperator && (cgaId != null)       // acts like Company Group Admin

// NEW: flags that PersonalData consumes (keep names same as EditCrewModal)
const operatorActsLikeSuperadmin = isSuperadminOperator
const operatorActsLikeGroupAdmin  = isCompanyOperator
...
const [generatedUsername, setGeneratedUsername] = useState<string>('');
  ...
  // Always start with fresh creds on a new Add Crew session
useEffect(() => {
  if (isOpen && !crewId) {
...
useEffect(() => {
  if (!isOpen) return;
  // wipe out any stale top error when the modal opens
  clearTopError();
}, [isOpen, clearTopError]);


const coreLocked = !!crewId && savedTabs.personal;
  ...
  // Ref to call saveAll() on FlagDocuments tab
  const flagDocsRef = useRef<{ saveAll: () => Promise<boolean> } | null>(null)
...
const additionalDetailsForm = React.useMemo(() => ({ ...additionalDetails, references }), [additionalDetails, references]);
...
const isAdditionalDetailsDirty = React.useMemo(
  () => JSON.stringify(additionalDetailsForm) !== additionalDetailsSnapshot,
  [additionalDetailsForm, additionalDetailsSnapshot]
);
  ...
  }, [activeTab, crewId, personalForm, documentForm, academicForm, watchkeepingForm, nextOfKinForm, coursesForm, seaServiceForm, additionalDetailsForm, savedTabs])
...
if (draft?.additionalDetailsForm) {
        ...
        setReferences(JSON.parse(draft.additionalDetailsForm.references || '[]'));
setSeaServiceSnapshot(JSON.stringify(draft?.seaServiceForm ?? []));
      setAdditionalDetailsSnapshot(JSON.stringify(draft?.additionalDetailsForm ?? {}));      }
...
const safeSetActiveTab = (tab: TabKey) => {
  // clear any stale “Failed to save …” message when navigating
  clearTopError();

  // Hard rule: you can’t leave Personal until a crew is created.
  if (!crewId && tab !== 'personal') {
    toast.error('Please save "Personal Data" first');
    setActiveTab('personal');
    return;
  }
  // Once a crewId exists, allow free navigation to any tab (even if previous tabs are unsaved).
  setActiveTab(tab);
};
...
if (activeTab === 'flagDocs' && crewId) {  // ← Added this
...
//   }
  ...
  try {
    ...
    // For Company (5) or Company-Operator: filter by their CGA.
    // For Superadmin (1) or Superadmin-Operator: don't pre-filter; list empty until a CGA is chosen in UI.
    let groupIdForFilter: number | null = null;
    if (roleId === 5 || isCompanyOperator) {
      groupIdForFilter = Number((roleId === 5 ? roleEntityId : cgaId) ?? 0);
    } else {
      groupIdForFilter = null; // Superadmin & Superadmin-operator & Subcompany don't use this direct list
    }

    const activeCompanies = groupIdForFilter
      ? companyList.filter(
  ...
  const emptySeaServiceRow = (serialNo: number): SeaServiceRequest => ({
    serialNo,
    companyName: '',
    vesselName: '',
    typeOfVesselFlag: '',
...
const handleRemoveSeaService = (index: number) => {
  setSeaServices((prev) => {
    // If it’s the only row, “delete” means reset it to empty
    if (prev.length === 1) {
      return [emptySeaServiceRow(1)];
    }
  ...
  const fetchVessels = async () => {
    try {
      ...
      if (roleId === 1 || isSuperadminOperator) {
        // Superadmin & Superadmin-operator => all active vessels
        return isActive
      } else if (roleId === 5 || isCompanyOperator) {
        // CGA & Company-operator => only vessels in their CGA
...
const validateAdditionalDetailsFields = () => {
  ...
  // // Validate pump experience months (optional, but must be non-negative if provided)
  // if (additionalDetails.pumpExperienceFramoMonths && additionalDetails.pumpExperienceFramoMonths < 0) {
  //   errors.pumpExperienceFramoMonths = 'FRAMO experience months cannot be negative';
  // }
  // if (additionalDetails.pumpExperienceCopMonths && additionalDetails.pumpExperienceCopMonths < 0) {
  //   errors.pumpExperienceCopMonths = 'COP experience months cannot be negative';
  // }

  // // Validate conditional fields only if their corresponding boolean is true
  ...
  // }
  // if (additionalDetails.courtOfEnquiryInvolvement && !additionalDetails.courtOfEnquiryDetails?.trim()) {
  ...
  // }
  // if (additionalDetails.certificateSuspendedRevoked && !additionalDetails.certificateSuspendedRevokedDetails?.trim()) {
  //   errors.certificateSuspendedRevokedDetails = 'Certificate suspension details are required when selected';
  // }
  // if (additionalDetails.hasMedicalConditions && !additionalDetails.medicalConditionsDetails?.trim()) {
  //   errors.medicalConditionsDetails = 'Medical condition details are required when selected';
  // }
  // if (additionalDetails.habitualUseDrugsAlcohol && !additionalDetails.habitualUseDetails?.trim()) {
  //   errors.habitualUseDetails = 'Habitual use details are required when selected';
  // }
  // if (additionalDetails.workedWithMultinational && !additionalDetails.multinationalNationalities?.trim()) {
  //   errors.multinationalNationalities = 'Nationalities are required when multinational workforce is selected';
  // }

  // // Validate referral source and details (optional, but details required if source is selected)
  ...
  // // Validate references only if hasReferenceData is true
  ...
  // }
    ...
    // --- Declarations: require all 6 to be confirmed ---
  ...
  // 6th declaration (No Agents) must be checked, and its date is required
  ...
  // Require only applicant name and date
  ...
  const handleDocumentSubmit = async () => {
    ...
    try {
      ...
      // before navigating forward, make sure any previous error is cleared
clearTopError(); 
  ...
  const handleAcademicSubmit = async () => {
    ...
    // if (!savedTabs.documents) {
    //   setError('Please save Documents first.')
    //   toast.error('Please save Documents first.')
...
clearTopError();
  ...
  const handleWatchkeepingSubmit = async () => {
    ...
    // if (!savedTabs.academic) {
    //   setError('Please save Academic Background first.')
    //   toast.error('Please save Academic Background first.')
    ...
    // }
        try {
      // Always use latest data so row-level uploads don't cause duplicates
      ...
      const updatedCertificates = await Promise.all(
        watchkeepingCertificates.map(async (certificate) => {
          ...
          } else if (certificate.certificateNo) {
      ...
      setWatchkeepingCertificates(
        certificateDetails.map((detail) => {
          const existingCert = validCertificates.find((cert) => cert.certificateDetails === detail)
          return {
            certificateDetails: detail,
            certificateNo: existingCert?.certificateNo || '',
            dateOfIssue: existingCert?.dateOfIssue || '',
  ...
  // if (!savedTabs.watchkeeping) {
  //   setError('Please save Watchkeeping Certificates first.');
  //   toast.error('Please save Watchkeeping Certificates first.');
  //   setActiveTab('watchkeeping');
  //   return;
  // }
...
const handleCourseCertificatesSubmit = async () => {
  ...
  // if (!savedTabs.kin) {
  //   setError('Please save Next of Kin Details first.');
  //   toast.error('Please save Next of Kin Details first.');
  //   setActiveTab('kin');
  //   return;
  // }
  ...
  // }
    try {
    // Refresh from backend to avoid duplicates after row-level uploads
    ...
    setCourseCertificatesSnapshot(JSON.stringify(courseCertificates))
    toast.success(
...
const handleSeaServiceSubmit = async () => {
  ...
  try {
    // ✅ Always refresh existing from backend before diffing (prevents mismatch)
    const freshExisting = await getSeaServices(crewId);
    setExistingSeaServices(freshExisting);

    const currentIds = seaServices.map(s => s.id).filter((x): x is number => typeof x === 'number');
    const deletedIds = freshExisting
      .map(s => s.id)
      .filter(id => !currentIds.includes(id));

    // ✅ delete removed rows in backend
    await Promise.all(deletedIds.map(id => deleteSeaService(crewId, id)));

    // ✅ upsert remaining rows
    const upserts = await Promise.all(
      seaServices.map(async (service, index) => {
        const seaServiceData: SeaServiceRequest = {
          serialNo: index + 1,
          companyName: service.companyName?.trim() ? service.companyName : null,
          vesselName: service.vesselName?.trim() ? service.vesselName : null,
          typeOfVesselFlag: service.typeOfVesselFlag?.trim() ? service.typeOfVesselFlag : null,
          grtdrt: service.grtdrt?.trim() ? service.grtdrt : null,
          engineType: service.engineType?.trim() ? service.engineType : null,
          kwtBhp: service.kwtBhp?.trim() ? service.kwtBhp : null,
          rank: service.rank?.trim() ? service.rank : null,
          fromDate: service.fromDate || null,
          toDate: service.toDate || null,
          totalMonthsDays: service.totalMonthsDays?.trim() ? service.totalMonthsDays : null,
          reasonForSignOff: service.reasonForSignOff?.trim() ? service.reasonForSignOff : null,
        };

        // Only save rows that have meaningful data (you can adjust rule)
        const hasAnyValue = !!seaServiceData.companyName;
        if (!hasAnyValue) return null;

        if (service.id) {
          return updateSeaService(crewId, service.id, seaServiceData);
        }
        return createSeaService(crewId, seaServiceData);
      })
    );

    // ✅ reload from backend so UI reflects true state
    const latest = await getSeaServices(crewId);
    setExistingSeaServices(latest);
    ...
    setSeaServices(mapped.length ? mapped : [emptySeaServiceRow(1)]);

    setSavedTabs((s) => ({ ...s, seaService: true }));
    setSeaServiceSnapshot(JSON.stringify(mapped));
    toast.success('Sea Services updated');
    clearTopError();
  ...
  const handleFlagDocsSubmit = async () => {
    clearTopError()
    if (!crewId) {
      setError('Please fill and save Personal Data first to create a crew member.')
      toast.error('Please fill and save Personal Data first.')
    ...
    if (!flagDocsRef.current) {
      toast.error('Flag Documents form is not ready.')
      return
    }

    try {
      const ok = await flagDocsRef.current.saveAll()
      if (!ok) {
        setError('Failed to save flag documents.')
        toast.error('Failed to save flag documents.')
        return
      }
      ...
      clearTopError()
      setActiveTab(nextTab('flagDocs')) // moves to "other"
    } catch (err) {
      console.error('Failed to save flag documents:', err)
      setError('Failed to save flag documents.')
      toast.error('Failed to save flag documents.')
    }
  }
...
const handleAdditionalDetailsSubmit = async () => {
  ...
  // if (!savedTabs.seaService) {
  //   setError('Please save Sea Service first.');
  //   toast.error('Please save Sea Service first.');
  //   setActiveTab('seaService');
  //   return;
  // }
  ...
  try {
    const additionalDetailsData: AdditionalDetailsRequest = {
      ...
      certificateSuspendedRevokedDetails: additionalDetails.certificateSuspendedRevokedDetails || null,
    ...
    //   updated = await updateAdditionalDetails(crewId, existingAdditionalDetails.id, additionalDetailsData);
    ...
    // before closing, make sure any previous error is cleared
clearTopError();
  ...
  const validateField = (field: string, value: any): string => {
    switch (field) {
      ...
      case 'companyAdmin':
        return roleId === 1 && !value.id ? 'Company selection is required' : ''
      case 'dateOfBirth':
...
const validateFields = () => {
  ...
  errors.dateOfBirth = !dob
    ...
    : (dob > cutoff ? 'You must be at least 18 years old as of today' : '')
  ...
  errors.willingToAcceptLowerRank = validateField('willingToAcceptLowerRank', willingToAcceptLowerRank)
  ...
  if ((roleId === 5 || isCompanyOperator) && companies.length > 0 && !selectedCompany.id) {
  ...
  // REPLACE the existing generateUsername with this robust one
const generateUsername = (crewName: string) => {
  const base = (crewName || 'crew')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '')

  // timestamp + cryptographically-strong random suffix => practically unique
  const ts = Date.now().toString(36)
  const rand = (() => {
    if (typeof window !== 'undefined' && window.crypto?.getRandomValues) {
      const bytes = new Uint8Array(6)
      window.crypto.getRandomValues(bytes)
      // base36-ish string (no custom colors or styles :)
      return Array.from(bytes, b => (b % 36).toString(36)).join('')
    }
    return Math.random().toString(36).slice(2, 8)
  })()

  return `${base || 'crew'}_${ts}${rand}`
}
  ...
  // --- helper near top of file (add once, outside the component or above handleSavePersonal) ---
const toYMD = (d: Date | null | undefined): string | undefined => {
  if (!d) return undefined;
  // make it timezone-agnostic "YYYY-MM-DD"
  const dt = new Date(d);
...
clearTopError();                 
  ...
  const mustPickSubCompany = (roleId === 5 || isCompanyOperator) && companies.length > 0
if (mustPickSubCompany && !selectedCompany.id) {
  ...
  try {
    ...
    // Determine companyGroupAdminId and companyAdminId based on role or operator flavor
let companyGroupAdminId: number
let companyAdminId: number | null = null

if (roleId === 1 || isSuperadminOperator) {
  // Superadmin & Superadmin-operator: must choose CGA; optional sub company
  if (!selectedCompanyAdmin.id) {
    toast.error('Please select a Company Group Admin.')
    return
  }
  companyGroupAdminId = Number(selectedCompanyAdmin.id)
  companyAdminId = selectedCompany.id || null
} else if (roleId === 5 || isCompanyOperator) {
  // CGA & Company-operator: CGA is fixed; optional sub company
  const resolvedCga = roleId === 5 ? roleEntityId : cgaId
  if (!resolvedCga) {
    toast.error('Invalid user configuration: Group Admin ID is missing.')
    return
  }
  companyGroupAdminId = Number(resolvedCga)
  companyAdminId = selectedCompany.id || null
} else if (roleId === 2) {
  // Company Admin: CGA from currentUser, Company is roleEntityId
  if (!currentUser?.companyGroupAdminId || !roleEntityId) {
    toast.error('Invalid user configuration for Sub Company.')
    return
  }
  companyGroupAdminId = Number(currentUser.companyGroupAdminId)
  ...
  return
    ...
    if (!crewId) {
      // ---------------- CREATE with auto-retry on duplicate username ----------------
      let attempts = 0
      let created: Crew | null = null
      let finalUsername = ''
      let finalPassword = ''
      ...
      while (attempts < 3 && !created) {
        ...
        try {
          const resp = await createCrew(
            ...
            username,
            password,
            Number(presentRankId),
            companyGroupAdminId,
            companyAdminId,
            certifications.filter((c) => c.file),
            documents.filter((d) => d.file),
            Number(presentRankId),
            rankAppliedForId ? Number(rankAppliedForId) : null,
            telNo,
            familyName,
            alternateMobNo,
            skypeId,
          ...
          const duplicate =
            /Duplicate entry/i.test(msg) && /UKr43af9ap4edm43mmtq01oddj6/i.test(msg)
          if (!duplicate) {
            throw err
          }
          attempts += 1
          // loop again with a new username
        }
      }
      ...
      toast.success('Personal Data saved. Crew created.')
      onCrewAdded({password: finalPassword, link: loginLink, created})
      ...
      await updateCrew(crewId, {
        ...
        contactNumber,
        email,
        rankId: Number(rankAppliedForId) || Number(presentRankId), // Use rankAppliedForId if available, else presentRankId
        presentRankId: Number(presentRankId),
        rankAppliedForId: rankAppliedForId ? Number(rankAppliedForId) : undefined,
        companyGroupAdminId, // Use corrected ID
        companyAdminId, // Use corrected ID
        active: true,
        notes,
        familyName,
        placeOfBirth,
...
clearTopError();
    ...
    const errorMessage = err.response?.data?.message || 'Failed to save Personal';
  ...
  const hasUnsavedChanges =
    isPersonalDirty || isDocumentDirty || isAcademicDirty || isWatchkeepingDirty || isNextOfKinDirty || isCoursesDirty || isSeaServiceDirty || isFlagsDirty || isAdditionalDetailsDirty
  useEffect(() => {
      ...
      >
        <div className='custom-modal-header d-flex justify-content-between align-items-center'>
          ...
          <span className='d-flex align-items-center gap-2'>
            {generatedPassword && savedTabs.personal && (
              <div className='d-flex align-items-center bg-light p-2 rounded '>
                <span className=' me-2'>Password for Crew Login:&nbsp; {generatedPassword}</span> &nbsp;
                <button
                ...
                >
                  {`  Copy  `}
                  {/* <KTSVG path='/media/map/copy.svg' className='svg-icon-2' /> */}
                </button>
              </div>
            )} 
  ...
  >
    Flag Document{' '}
    {savedTabs.flagDocs && <span className='text-success ms-1'>✓</span>}
  </button>
</li>
          <li className='nav-item'>
            <button
...
{activeTab === 'flagDocs' && (
  ...
  {activeTab === 'personal' && (
    <div className='d-flex gap-2 justify-content-end mt-4'>
      <button
        ...
        disabled={crewId ? !isPersonalDirty : false}
      >
        {crewId ? 'Save & Next →' : 'Create & Next →'}
      </button>
    </div>
  )}
  {activeTab === 'documents' && (
  ...
  <div className='d-flex gap-2 justify-content-end mt-4'>
    ...
    <button
      className='btn btn-primary'
      onClick={handleFlagDocsSubmit}
      disabled={!isFlagsDirty}
    >
      {savedTabs.flagDocs ? 'Update & Next →' : 'Save & Next →'}
    </button>
  </div>
)}
  {activeTab === 'other' && (
  <div className='d-flex gap-2 justify-content-end mt-4'>
    ...
    >
      {existingAdditionalDetails?.id ? 'Update & Finish' : 'Save & Finish'}
    </button>
  </div>
)}
</div>
